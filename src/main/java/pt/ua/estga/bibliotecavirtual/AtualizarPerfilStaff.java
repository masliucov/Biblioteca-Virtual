...
private void atualizarEmail() {
    ...
    if (!novoEmail.equals(SessaoUtilizador.getEmail())) {
        ...
        try (Connection conn = DatabaseUtil.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("SELECT EXISTS(SELECT 1 FROM utilizador WHERE email = ?) AS 'exists'")) {
    ...
    try (Connection conn = DatabaseUtil.getConnection()) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            if (affectedRows > 0) {
                ...
                JOptionPane.showMessageDialog(this, "Erro ao atualizar contato!");
            }
        }
    } catch (SQLException ex) {
        JOptionPane.showMessageDialog(this, "Erro de SQL: " + ex.getMessage());
...
private void atualizarCampo(String campo, String valor) {
    try (Connection conn = DatabaseUtil.getConnection();
         PreparedStatement pstmt = conn.prepareStatement("UPDATE utilizador SET " + campo + " = ? WHERE id = ?")) {
